...
const schema = yup.object().shape({
    ...
    username: yup.string().required('Không được để trống.').matches(/^[a-z]+$/, 'Tên tài khoản không được chứa số, ký tự đặc biệt').min(6, 'Chứa ít nhất 6 ký tự'),
    password: yup.string().required('Không được để trống.').min(8,'Mật khẩu phải lớn hơn 8 ký tự').trim(),
    ...
    phone: yup.string().required('Không được để trống.').matches(/^[^\s]*(84|0[3|5|7|8|9])+([0-9]{8})\b/,"Sai định dạng SĐT").trim(),
    ...
    dateOfBirth:yup.date().nullable().required('Không được để trống.').typeError('Vui lòng nhập định dạng dd/MM/yyyy'),
